"""Standalone backup-restore command with optional snapshot_id (latest if omitted)."""
...
def make_backup_restore_command(bot: discord.Client, db_path: str):
    """Build an app_commands.Command with optional snapshot_id."""
    ...
    async def backup_restore(
        ...
        async with aiosqlite.connect(db_path) as db:
            if resolved_id is not None:
                ...
                async with db.execute(
                    """
                    SELECT id, name, data FROM snapshots
        ...
        cog._restore_task = asyncio_create_task(
    ...
    # Fix: use asyncio at module level properly
    return backup_restore


# Fix the create_task reference properly in the command body
import asyncio


def register_backup_restore(bot, db_path: str) -> app_commands.Command:
    """Create and return the backup-restore command (optional snapshot_id)."""
